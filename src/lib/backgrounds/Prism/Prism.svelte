<script lang="ts">
	...
	function createPrism(node: HTMLDivElement) {
		container = node;
    ...
    `;
		...
		const render = (t: number) => {
			...
			// Update reactive props in the animation loop
			program.uniforms.uHeight.value = Math.max(0.001, height);
			program.uniforms.uBaseHalf.value = Math.max(0.001, baseWidth) * 0.5;
			...
			program.uniforms.uScale.value = Math.max(0.001, scale);
			...
			if (animationType === 'hover') {
				const maxPitch = 0.6 * Math.max(0, hoverStrength || 1);
				const maxYaw = 0.6 * Math.max(0, hoverStrength || 1);
				...
				if (Math.max(0, timeScale || 1) < 1e-6) continueRAF = false;
			}
...
<div
	class="prism-container {className}"
